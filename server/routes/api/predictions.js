    ...
    if (req.query.fightID && req.query.userID) {
      Predictions.findOne({
        ...
        .catch(() => {
          return res
            .status(400)
            .json({ error: "Error in finding predictions for user" });
        });
    }
      ...
      Predictions.find({
        ...
        .then((prediction) => {
          if (!prediction) {
            ...
            return res
              .status(200)
              .json({ success: true, predictions: prediction });
          }
        })
        .catch(() => {
          return res
  ...
  (req, res) => {
    ...
    newPrediction
      ...
      .catch(() => {
        return res
          .status(400)
          .json({ error: "Error creating a new prediction" });
      });
  }
);
...
router.delete(
  ...
  (req, res) => {
    console.log("================");
    console.log(req.body.predictionID);
    console.log("===============");
    Predictions.deleteOne({
      ...
      .then(() => {
        ...
        return res.status(200).json({ status: "ok" });
        //   &&
        //   this.$router.push("pickFighter")
      })
      .catch(() => {
        return res
          .status(400)
          .json({ error: "Error deleting a new prediction" });
      });
  }
);
