...
const actions = {
  ...
  async createInitialPosts({ commit }, newIP) {
    ...
    console.log(newIP);
    let res = await axios.post(url + "/initialpost/searchresults", newIP);
  ...
  async fetchRatings({ commit }, IPid) {
    const response = await axios.get(
      url + "/initialpost/searchresults/landlordratings/" + IPid
    );
  ...
  async newRating({ commit }, IPid, Rating) {
    const response = await axios.post(
      url +
        "/initialpost/searchresults/landlordratings/" +
        IPid +
        "/createreview",
      Rating
    );
...
const mutations = {
  ...
  setRatingsResults: (state, queryofRatings) =>
    (state.queryofRatings = queryofRatings),
};
